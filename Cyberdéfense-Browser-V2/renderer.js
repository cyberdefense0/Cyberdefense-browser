...
function createTab(url = 'about:blank', title = 'Nouvel onglet') {
  ...
  webview.addEventListener('dom-ready', () => {
    try {
      webview.insertCSS(`
        ...
        #app, .app, .container, .main, .page, .content {
          width: 100% !important;
          min-width: 100% !important;
          min-height: 100vh !important;
          height: 100% !important;
        ...
        .wrapper, .content-wrapper, .page-content {
          min-height: 100vh !important;
          height: 100% !important;
        }
      `);
    } catch (error) {
      console.warn('Impossible d’injecter le CSS du webview:', error);
    }
  });
  
  // Événements du webview
  ...
  webview.addEventListener('did-finish-load', async () => {
    if (activeTabId === tabId) {
      ...
      try {
        const pageTitle = await webview.executeJavaScript('document.title') || 'Sans titre';
        tab.title = pageTitle;
        updateTabTitle(tabId, pageTitle);
        currentTabTitle.textContent = pageTitle;
      } catch (error) {
        tab.title = currentUrl;
        updateTabTitle(tabId, currentUrl);
        currentTabTitle.textContent = currentUrl;
      }
      
      if (pageLoadTime > 0) {
        pageLoadTime = Date.now() - pageLoadTime;
        performanceData.push({url: currentUrl, time: pageLoadTime, timestamp: new Date()});
      }
  ...
  webview.addEventListener('did-fail-load', (event) => {
    if (activeTabId === tabId) {
      const failedUrl = event.validatedURL || url;
...
function switchToTab(tabId) {
  ...
  // Afficher le webview de l'onglet actif
  const tab = tabs.find(t => t.id === tabId);
  if (tab && tab.webview) {
    tab.webview.style.display = 'block';
    resizeWebview(tab.webview);
    resizeAllWebviews();
    console.log('✓ Webview affiché:', tabId, 'Dimensions:', tab.webview.offsetWidth, 'x', tab.webview.offsetHeight);
    activeTabId = tabId;
    ...
    document.querySelectorAll('.tab-button').forEach(btn => {
      btn.classList.remove('bg-cyber/20', 'text-cyber');
      btn.classList.add('bg-slate-800', 'text-slate-300');
    });
    
    const activeTabButton = document.querySelector(`[data-tab-id="${tabId}"]`);
    if (activeTabButton) {
      activeTabButton.classList.remove('bg-slate-800', 'text-slate-300');
      activeTabButton.classList.add('bg-cyber/20', 'text-cyber');
    }
    
    updateSecurityPanel(tab.url);
    log('Onglet actif: ' + tab.title);
  } else {
    console.error('❌ Tab introuvable ou pas de webview:', tabId);
  }
}
...
function renderTabs() {
  ...
  tabs.forEach(tab => {
    ...
    tabButton.className = 'tab-button flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800 text-slate-300 hover:bg-slate-700 cursor-pointer transition-colors min-w-0 flex-1 max-w-48';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  trackerToggle.addEventListener('click', async () => {
    ...
    const activeWebview = getActiveWebview();
    updateSecurityPanel(activeWebview ? await activeWebview.getURL() : addressBar.value);
  });
  ...
  addFavoriteButton.addEventListener('click', async () => {
    ...
    const currentUrl = await activeWebview.getURL();
